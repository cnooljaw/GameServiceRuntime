...
- Battle 当前牌规层已按参考覆盖单牌、对子、三张、三带二和 4～8 张炸弹；A/2 使用旧逻辑值，炸弹优先且同型炸弹按长度比较；发牌使用两副完整 1..K 牌，双副中的相同物理牌可同时出。
...
2. 完整 104 张牌的随机/新手/散牌调整、名次、单扣/双扣和完整结算；当前已完成最小牌型识别、比较、真实双副牌集合和当前墩抓分/TurnEnd。
