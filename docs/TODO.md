...
| CARD-024 | 进行中 | 隔离 Battle 随机数与时间 | NHSK Battle 已注入独立 PRNG 与 `NHSKClock`；生产 seed 只来自 `crypto/rand`，读取失败则创建失败，不使用全局随机或时间降级；测试可注入固定 seed/fake Clock，普通洗牌、庄家抽取、期限起点和剩余时间判断已只使用 Battle 依赖。回放结束时间、诊断材料和其他尚未接入的时间事实仍待后续切片；旧回放 XML 不增加字段。 |
...
| CARD-032 | 已确认待实现 | 固化操作 fencing 与输出线序 | VerifyCode 为 3/5/7 序列；开局 GameInfo→私有 Deal→Ask。玩家坏动作只定向失败且不重置期限，AI/托管坏候选静默；合法动作无成功 ACK，按可见性输出对家亮牌、OutCardInfo；当前已补三家过牌后的 TurnEnd 与下一 Ask 线序。终局按 OutCardInfo→全桌 ShowCards→0x8650→客户端 GameResult→回放→GM GameOver 验收，目标、隐藏信息和批次顺序不得改变。 |
...
| CARD-054 | 进行中 | 固化回放 Moves 与序列化 | 已完成 Battle-owned 内存 `ReplayMove`：Deal 只计一个事件；保留 CurrentPoint→OutCard 和 OutCard→CatchPoint→TurnEnd 旧线序、来源与中文牌型（俘虏、炸弹 N）。领域类型使用 `ReplayMoveSource`，只有 XML serializer 写旧属性名 `Actor`。仍需补齐 MoveMilliseconds 来源、XML Header/Tab/属性排序、小写牌值和真实 Moves.Count；不实现无调用的 PickCard/Offline/Reconnect/任意 AddMove。 |
...
| CARD-057 | 进行中 | 冻结每局玩家与发牌快照 | 已完成 `ReplayDocument`/`ReplayStartSnapshot` 的深拷贝内存模型：StartSubgame 冻结 Seat/User/NickName/InitScore/CltID(Platform)/Dress/Automated、RoundContext、同一批最终四手牌和起始时间；START 后 UPDATE_PLAYER 只影响下一局。Deal 与客户端私有发牌仍需接入同一快照，ReplayUID、规范 ReplayName/目录、终局树和序列化继续由 CARD-052～058 完成；CntID 不进回放。 |
