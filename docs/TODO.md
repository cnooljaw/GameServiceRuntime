...
| CARD-025 | 进行中 | 归一化实际可达的 NHSK 规则 | Legacy `INIT_GAME` 已解码连续 `BaseRule/GameRule/MatchName` suffix；当前只把 GameRule 的最小机器人出牌次数、比例和单牌换牌数量投影到不可变 `NHSKConfig`，出牌期限保留参考默认并允许直接 Cluster 配置。缺失、空值或坏值保持默认，多余字段忽略。偏置洗牌及其他已放弃能力不建立字段、handler 或占位接口。普通散牌和 `NEW_GAME.IsNewbie` 新手调整已接入；仍需自定义牌堆等独立 provider。 |
...
| CARD-031 | 进行中 | 复刻 NHSK 核心牌规与结算 | 已完成参考牌型的最小识别/比较：单、对、三、三带二、4～8 张炸弹，A/2 逻辑值、炸弹优先及同型炸弹长度比较；普通发牌已使用 Battle 独立 PRNG、单次庄家抽取、洗牌、旧 SwapSingleCard 散牌调整、新手三张/四张重试和庄家环形分发，两副完整 1..K 牌每座 26 张；当前墩已累计 5/10/K 抓分，三家过牌后按参考输出 TurnEnd、归属分值并清空本墩状态。仍待完成自定义牌堆、名次、单扣/双扣、回放及完整综合结算。 |
...
| CARD-036 | 进行中 | 新手发牌兼容切片 | 已保留 `NEW_GAME.IsNewbie` 并由 Host 传入 Battle；无自定义牌堆时，Battle 在普通确定性洗牌后按座位顺序选择首个非 `Automated` 玩家，执行旧 `RandCardListByNewPlayer` 的三张/四张重试，全自动玩家跳过；固定 seed 测试锁定四手牌结果。未引入 Nacos、每座偏牌配置或通用偏洗牌。自定义牌堆 provider 尚未接入。 |
