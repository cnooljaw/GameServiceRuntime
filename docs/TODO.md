...
| CARD-024 | 进行中 | 隔离 Battle 随机数与时间 | NHSK Battle 已注入独立 PRNG 与 `NHSKClock`；生产 seed 只来自 `crypto/rand`，读取失败则创建失败，不使用全局随机或时间降级；测试可注入固定 seed/fake Clock，普通洗牌、庄家抽取、期限起点、场景剩余时间和 Ask→合法动作的回放毫秒数已只使用 Battle 依赖。回放结束时间、诊断材料和其他尚未接入的时间事实仍待后续切片；旧回放 XML 不增加字段。 |
...
| CARD-060 | 进行中 | 统一综合结算双入口 | Battle 已在终局产生类型化 SettlementRequestOutput，Legacy egress 精确编码 `0x8650` 固定体及 gain/player 两段 suffix；BSAck 仍以 Send 映射到 CompleteSettlement，Cluster Send/Call 共用同一 typed request 和矩阵门禁。Call 不复制异步输出。仍需删除早期 `Scores[4]` 兼容形状，并把回放 writer/ROUND_STAT/GAME_OVER 收敛接完。 |
...
| CARD-031 | 进行中 | 复刻 NHSK 核心牌规与结算 | 牌型、发牌、自定义牌堆与逐墩抓分已完成；对家出完时现在会先结清桌面分牌，再按参考 `100/105/200` 分支计算名次、单扣/双扣、胜组、倍数和定向结算矩阵，并产生 SettlementRequestOutput。仍需完整回放终局、AI/超时来源和 MATCH_STOP 本地收尾。 |
| CARD-032 | 进行中 | 固化操作 fencing 与输出线序 | 已实现 VerifyCode 3/5/7 序列，以及开局 GameInfo→四份私有 Deal→Ask；每份 Deal 与回放 Deal 复用 Battle 最终手牌。玩家坏动作只定向失败且不重置期限或回放行动起点；合法动作无成功 ACK，三家过牌后保持 TurnEnd→下一 Ask。仍需 AI/托管坏候选、完整对家亮牌与终局 OutCardInfo→ShowCards→0x8650→GameResult→回放→GameOver 验收。 |
...
| CARD-038 | 进行中 | 托管结算认定与负分修正 | 已按合法动作来源累计 MoveCount/AutoCount/MoveMS，GameRule 前两项已改为领域名并保留 -1、单项/双项和乘法公式；成功结算已按参考把单个托管失败队员的搭档负分转移给托管者，SettlementRequest 与客户端 GameResult 共用认定。仍需 AI/普通超时/AI 超时来源接入。 |
...
| CARD-043 | 进行中 | 小局生命周期输出兼容 | 开局已严格输出 `0x7205 GAME_START -> 0x8654 GAME_STARTED -> NHSK GameInfo -> Seat0..3 私有 Deal -> Ask`；不实现无调用点的 `0x7206 GAME_END`。仍需在客户端结果和回放后接入 PlayerCount=0 的 `0x7246 ROUND_STAT` 与完整座位序 `GAME_OVER`；`NOTICE_ROUND_OVER` 继续只用于 MATCH_STOP 强制收尾且位于 GAME_OVER 后。 |
...
| CARD-052 | 进行中 | 隔离回放规则与文本兼容投影 | ReplayRuleSnapshot 已只从 BaseRule 投影 TimeOutOver/VoiceMode/RandomSeatRoundStart/GameNumToRandomSeat，且不进入玩法。仍需完整 Info builder 接入 TimeoutAutoMove、SecRoundTotal 的 GameNum/GameTime 分支、稳定 Players/Dress 与隔离的 UTF-8/GBK 文本转换。 |
...
| CARD-054 | 进行中 | 固化回放 Moves 与序列化 | 已完成 Battle-owned `ReplayMove` 和纯内存 Moves serializer：Deal 只计一个事件并生成 D0..D3；保留 CurrentPoint→OutCard、OutCard→CatchPoint→TurnEnd、MoveMilliseconds、旧中文牌型和来源。serializer 固定 XML Header、Tab、字典序属性、小写牌值、真实 Count，并只在边界写旧 `Actor` 中文值。仍需把同一节点接入完整 ReplayBuilder；不实现无调用的 PickCard/Offline/Reconnect/任意 AddMove。 |
...
| CARD-057 | 进行中 | 冻结每局玩家与发牌快照 | 已完成 `ReplayDocument`/`ReplayStartSnapshot` 的深拷贝内存模型：StartSubgame 冻结 Seat/User/NickName/InitScore/CltID(Platform)/Dress/Automated、RoundContext、同一批最终四手牌和起始时间；四份客户端私有 Deal 与回放 Deal 已复用这批 Battle 最终手牌，START 后 UPDATE_PLAYER 只影响下一局。ReplayUID、规范 ReplayName/目录、终局树和序列化继续由 CARD-052～058 完成；CntID 不进回放。 |
| CARD-058 | 进行中 | 分离 ReplayName 与 ReplayUID | START 已分别冻结 NHSK 固定前缀的 ReplayName 与 Unix秒+CreatorID 的 ReplayUID，并让 GAME_STARTED/当前 GAME_OVER 复用同一 ReplayName；仍需把 UID 接入完整 XML Info 与客户端 FuPanUID[64]。两者不互相推导，不增加防碰撞机制。 |
