...
| CARD-022 | 进行中 | 隔离自定义牌堆数据源 | 已实现本地文件/Redis `CustomDeckProvider`、宽松旧 grammar、账号白名单、可配置有界 `CustomDeckRunner`、每小局一次异步提交、超时/队列满/失败回退和 BattleRef/局号 fencing；自定义牌堆优先覆盖庄家与最终手牌，并绕过新手/散牌调整。Redis adapter 使用标准库 RESP GET，保持 `game:makecard:<ProductID>` 优先、空值回退 `<GameID>`；后续仍需真实 Redis 联调与完整 Legacy Deal/Replay golden。 |
