...
| CARD-054 | 进行中 | 固化回放 Moves 与序列化 | 已完成 Battle-owned 内存 `ReplayMove`：Deal 只计一个事件；保留 CurrentPoint→OutCard 和 OutCard→CatchPoint→TurnEnd 旧线序、来源与中文牌型（俘虏、炸弹 N）。OutCard 的 MoveMilliseconds 使用独立 Ask 起点与注入 Clock，非法动作和中途启用托管不重置。领域类型使用 `ReplayMoveSource`，只有 XML serializer 写旧属性名 `Actor`。仍需 XML Header/Tab/属性排序、小写牌值和真实 Moves.Count；不实现无调用的 PickCard/Offline/Reconnect/任意 AddMove。 |
