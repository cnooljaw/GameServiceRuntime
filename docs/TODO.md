...
| CARD-054 | 进行中 | 固化回放 Moves 与序列化 | 已完成 Battle-owned 内存 `ReplayMove`：Deal 只计一个事件；保留 CurrentPoint→OutCard 和 OutCard→CatchPoint→TurnEnd 旧线序、字段、Actor 与中文牌型（俘虏、炸弹 N）。仍需补齐 MoveMilliseconds 来源、XML Header/Tab/属性排序、小写牌值和真实 Moves.Count；不实现无调用的 PickCard/Offline/Reconnect/任意 AddMove。 |
