...
| CARD-028 | 进行中 | 以 DEL_GAME 停止屏障回收正常 Battle | 正常 Battle 的 Host 精确 Ref 先进入 Stopping；固定 runner Call 同一 Battle 的 Mailbox 屏障，屏障递增 TurnRevision、清除 deadline、禁止输出并 fence AI/回放/结算迟到结果，再 Runtime Stop，成功后才删除绑定。屏障或 Stop 失败保留绑定。Quarantined 保留规则随隔离切片收口。 |
