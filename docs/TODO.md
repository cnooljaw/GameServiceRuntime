...
| CARD-024 | 进行中 | 隔离 Battle 随机数与时间 | NHSK Battle 已注入独立 PRNG；生产 seed 只来自 `crypto/rand`，读取失败则创建失败，不使用全局随机或时间降级；测试可注入固定 seed，普通洗牌与庄家抽取已只使用该源。Clock 注入和所有时间判断仍待后续切片；诊断记录 seed 和有序输入，旧回放 XML 不增加字段。 |
