...
| CARD-018 | 进行中 | 统一 Legacy 与 Cluster 玩法入口 | Legacy TCP 与 Cluster Send/Call 都转换为同一套类型化 Command，进入同一个 Battle Mailbox，不设入口优先级；由状态、TurnRevision、VerifyCode 等业务 fencing 拒绝迟到请求。当前已完成 relay、GM 控制面、Host Resolve、Battle Mailbox、代际 OutputService，以及普通结算的 GameResult→回放→ROUND_STAT→完整 GAME_OVER 线序；仍需 MATCH_STOP、AI/超时和隔离状态后才能关闭。 |
...
| CARD-026 | 进行中 | 建立综合结算单飞状态机 | 已完成 Legacy `0x8650` 两段 suffix 解码、整包原子门禁、Flag 应用及失败 Dissolve 收敛；正确响应依次提交客户端 GameResult、不可变回放、ROUND_STAT 和带四座数据的 GAME_OVER。writer 结果以 BattleID/GameNum/SubgameNum/ReplayName fencing，错配、重复和迟到结果不重复输出。仍需 MATCH_STOP 与 DEL_GAME 竞争测试。 |
| CARD-060 | 进行中 | 统一综合结算双入口 | Battle 已在终局产生类型化 SettlementRequestOutput，Legacy egress 精确编码 `0x8650` 固定体及两段 suffix；BSAck 以 Send 映射到 CompleteSettlement，Cluster Send/Call 共用同一 typed request 和矩阵门禁，Call 不复制异步输出。回放 writer、ROUND_STAT 和完整 GAME_OVER 已接通；仍需删除早期 `Scores[4]` 兼容输入并覆盖 MATCH_STOP 竞争。 |
...
| CARD-043 | 进行中 | 小局生命周期输出兼容 | 开局已严格输出 `0x7205 GAME_START -> 0x8654 GAME_STARTED -> NHSK GameInfo -> Seat0..3 私有 Deal -> Ask`；普通终局已在客户端结果和回放后发送 PlayerCount=0 的 `0x7246 ROUND_STAT` 与 Seat0..3 完整 `GAME_OVER`。不实现无调用点的 `0x7206 GAME_END`；仍需 MATCH_STOP 路径在 GAME_OVER 后发送 NOTICE_ROUND_OVER。 |
...
| CARD-056 | 进行中 | 按来源编码回放文本 | MatchName/UserName 已使用隔离的 UTF-8-or-GBK helper，GameName 来自 descriptor；RoomInfo 空值省略、非空原样写 Json，Dress/RoomInfo 不全局转码，XML 使用标准转义且不记录原文。Prop 文本与目标顺序待 CARD-040 接入。 |
