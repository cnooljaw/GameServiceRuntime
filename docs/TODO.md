...
| CARD-031 | 进行中 | 复刻 NHSK 核心牌规与结算 | 已完成参考牌型的最小识别/比较：单、对、三、三带二、4～8 张炸弹，A/2 逻辑值、炸弹优先及同型炸弹长度比较；发牌改为两副完整 1..K 牌、每座 26 张；当前墩已累计 5/10/K 抓分，三家过牌后按参考输出 TurnEnd、归属分值并清空本墩状态。仍待完成随机/新手/散牌调整、名次、单扣/双扣、回放及完整综合结算；普通牌堆 PRNG/自定义牌堆按 CARD-022/CARD-024 后续接入。 |
