...
| CARD-031 | 进行中 | 复刻 NHSK 核心牌规与结算 | 已完成参考牌型的最小识别/比较：单、对、三、三带二、4～8 张炸弹，A/2 逻辑值、炸弹优先及同型炸弹长度比较；普通发牌已使用 Battle 独立 PRNG、单次庄家抽取、洗牌、旧 SwapSingleCard 散牌调整、新手三张/四张重试和庄家环形分发，两副完整 1..K 牌每座 26 张；自定义牌堆已在 Preparing 外部装载后覆盖庄家和最终手牌；当前墩已累计 5/10/K 抓分，三家过牌后按参考输出 TurnEnd、归属分值并清空本墩状态。仍待完成名次、单扣/双扣、回放及完整综合结算。 |
