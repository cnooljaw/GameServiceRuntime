...
| CARD-018 | 已确认待实现 | 统一 Legacy 与 Cluster 玩法入口 | Legacy TCP 与 Cluster Send/Call 都转换为同一套类型化 Command，进入同一个 Battle Mailbox，不设入口优先级；由状态、TurnRevision、VerifyCode 等业务 fencing 拒绝迟到请求。第一阶段 Cluster 调用者视为受信任内部服务，但玩家动作仍携带并校验本局 `UserID`。Cluster Call 只返回类型化结果和拒绝原因；客户端回复与广播仍只经 GameOutputService 发往旧 GM。GM 断线后普通 Battle 收敛并拒绝 Cluster 操作，隔离 Battle 返回隔离错误；Cluster 不能绕过 ConnectionGeneration 恢复或迁移旧局。 |
...
| CARD-020 | 已确认待实现 | 保持 Legacy 重连并提供只读 Snapshot | `USER_RECONNECT` 使用 `ReconnectPlayer`：清除 Offline，只有 playing 时退出托管并恢复视图；`GAME_SCENE` 使用独立 `RequestGameScene`：要求有效 game/subgame，不清除 Offline，退出托管并恢复视图。两者共用 `RestorePlayerView`，保留设置 ClientReady、GameInfo/Scene/当前询问和亮牌阶段全桌广播等当前生产输出顺序、目标与隐藏信息；生产关闭的解说时间与无目标规则证据的战绩不实现。Cluster `GetBattleSnapshot` 是独立纯查询，共用 Snapshot 构造但不改状态、不输出。无效 Legacy 请求保持静默；隔离 Battle 不执行副作用或伪造输出。分别以两条参考路径的 golden 和完整场景验证不影响原功能。 |
| CARD-021 | 进行中 | 建立 NHSK 切换范围门禁 | 已建立能力矩阵、Legacy MessageID 矩阵和逐切片参考核对记录；外部 AI、回放和自定义牌堆契约已冻结，未使用能力已放弃。`baison_middle/protocol v1.0.5` 已确认 24 字节 BSHeader 的全部字段、偏移和小端序，`BSSUFFIXIDX=8` 且 suffix 使用绝对偏移；上层 `protocol` 已确认 relay 布局，`nbgame_core` 已确认 Header.Length framing、严格出站长度与 8 KiB 上限。剩余门禁是固化双向 origin、普通 relay、旧 relay和综合结算的代表性整包 golden。 |
...
| CARD-026 | 已确认待实现 | 建立综合结算单飞状态机 | 小局按 `Playing -> AwaitingSettlement -> FinalizingReplay -> SubgameFinished` 推进；只发送一次 `0x8650`，等待期间拒绝玩法和下一小局。成功响应须全量覆盖四名冻结玩家、TeamID=SeatID、TeamCount=4，交易引用有效、正分且有向键唯一；坏内容整包拒绝并保持等待，不部分结算。只消费身份、Flag 与 ResultDetail 交易矩阵；PlayerData.Score/Exp、ResultType 不进领域，TeamCount 只校验。失败响应忽略 payload，按 Dissolve(4) 生成平局零分并完成客户端结果、回放、GAME_OVER，内部记录 SettlementFailed；重复/迟到响应和随后 MATCH_STOP no-op。不增加自动重试、超时、RequestID 或额外关联号。GM 断线仍终止未隔离 Battle。 |
| CARD-060 | 已确认待实现 | 统一综合结算双入口 | Battle 只实现 CompleteSettlement handler。Legacy BSAck 0x8650 以 Send 投递；Cluster Send/Call 进入同一 Command，Call 只返回 Accepted/Rejection/Revision，不复制异步输出。SettlementRequestOutput 是独立请求事实，不绑 Session；未来协调 Service 完成后回投 Command，Battle 不阻塞 Call 外部结算。 |
...
| CARD-031 | 已确认待实现 | 复刻 NHSK 核心牌规与结算 | 两副无王 52 张牌、每座 26 张；只实现单/对/三/三带二/4～8 张炸弹及现有比较规则。保留固定对家、过牌跳过、墩分归属、对家均出完结束，以及 `100/105/200` 阈值的名次和单双扣算法。普通庄家使用 Battle PRNG，自定义牌堆可覆盖；每小局只重置并抽取一次，不复制参考双 Reset。全部以表驱动和确定性整局 golden 验收。 |
...
| CARD-012 | 已确认待实现 | 原位替换 GameLogic | 按 RFC-0410 实现 NHSK 进程组合根、单条主动 GM 连接、双向 origin、连接代际、Host/Battle/GameOutput Service 和最小 Legacy allowlist。入站只解码已确认的 `0x8605` 控制消息与其中 `0x7402` relay；出站只编码可达的 `0x8644 + 0x7400` 客户端消息和已确认 GM 控制消息，不实现 `0x8655 GAME_MSG_OLD`。BattleID、异步生命周期、输出失败、重连和隔离语义均以 RFC-0410 为准。 |
