...
Spec 轴：逐条核对 RFC-0220 的 13 条验收、两阶段发布、尝试/窗口、Source/Generation、Snapshot 和 fail-closed。发现问题先写失败测试再修复。

Review 首轮发现并已修复：

- Standards：Client/Runner/Launcher 未统一拒绝 nil context；Client 未校验未知 `RecoveryFailure`；自定义 Launcher 返回部分 Ref 和 error 时 Runner 未 Abort。
- Spec：Publish 与 committed 之间再次 panic 时 prepared Generation 未 fencing；连续 CreateService 失败缺少完整 Runner/Supervisor 预算测试。
- 修复后重新执行双轴核对，无剩余 Standards 或 Spec finding。残余风险是第一版明确不提供持久失败通知和跨节点 Supervisor。
