...
```text
LoginService 负责认证和 secret 交换。
Gateway Adapter 负责连接验证、绑定、收发包。
...
`LoginService` 属于 Runtime Tooling。它提供通用登录编排，但不写死账号库、平台 SDK、渠道规则。
...
`AuthProvider` 属于 Business Layer 或业务 adapter。
...
LoginService
  ↓ 8. AuthProvider.VerifyLoginToken
LoginService
  ↓ 9. issue LoginTicket(uid, subid, SecretRef, expiresAt)
Client
...
`secret` 的交换只发生在第 1 到第 9 步。第 10 步以后只验证客户端知道这个 `secret`。
...
`SecretRef` 是密钥引用，不是明文密钥。明文 `secret` 只能存在于 LoginService、SessionRegistry 或 Gateway 的受控内存中。
...
- 生成 challenge。
- 完成密钥协商。
- 校验 HMAC，确认客户端持有同一个 `secret`。
- 解密或解析登录 token。
- 调用业务 `AuthProvider`。
- 处理单点登录、多端登录或顶号策略。
- 签发 `LoginTicket`。
- 把 `secret` 交给 `SessionRegistry` 或目标 Gateway。
