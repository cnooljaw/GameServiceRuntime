...
- `ReplayDocument` 起始时创建一个包含四手最终牌的单一 `Deal` 内存事件；合法出牌按参考顺序记录 `CurrentPoint -> OutCard`，每次出牌/过牌都记录 `OutCard`，三家过牌结束一墩时记录 `CatchPoint -> TurnEnd`。事件和牌区都通过深拷贝返回；当前 MoveMilliseconds、终局 Summary/CardDetail、XML 属性树和 writer 仍未接入。
