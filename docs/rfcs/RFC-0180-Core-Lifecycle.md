...
Status = Running
  ↓
Enqueue optional StartupCommand
  ↓
Return ServiceRef
```
...
Runtime 关闭会先阻止新的 Service 和 Runner 创建，关闭 Runtime 拥有的 Runner，再等待已经进入 `Init` 的创建流程完成。关闭 Runner 会取消并等待固定 worker 的真实返回；期限内未返回的 worker 继续通过 Runtime Inspection 可见。`Init` 失败或在 Runtime 进入 Closing 后才完成时，Runtime 仍调用该 Service 的 `Close()`，释放初始化过程中已取得的部分资源。
...
20. 启动后工作只能经可选 StartupCommand 进入 Mailbox；Runtime 不得在 `Init` 成功后直接调用 Service 的业务方法。
21. Runtime.Close 必须先关闭 Core Runner，再停止 Service；关闭超时不能遗失仍在执行的 Runner worker。
