...
- Moves 的生产入口进一步收窄为真实调用：一个 Deal Move 下按 SeatID 0..3 写 D0..D3（ChairID、UserID、Cards），四个子节点在 Moves.Count 中只计一次。有分牌的合法动作先写 CurrentPoint（当前累计分牌 Cards/Point、来源为系统），再写 OutCard；过牌只写 Cards 为空、CardType=不出的 OutCard。一墩结束在最后 OutCard 后，仅当分值大于 0 时写 CatchPoint（获分 ChairID、Cards、Point、来源为系统），再写 TurnEnd（逗号分隔的四座 Scores、来源为系统）。OutCard 另写 ChairID、从 Ask 到该合法动作的 MSec 和中文 CardType；人工、外部 AI、普通或 AI 超时、托管的来源分别是玩家、AI、超时、托管。领域模型以 `ReplayMove.Source/ReplayMoveSource` 表达来源，不使用 Actor 类型、字段或参数；XML serializer 只在兼容输出边界把来源映射为旧属性名 `Actor` 及上述中文值。牌型名称精确保留不出、单张、对子、三张、俘虏和按张数命名的炸弹4..8。
...
- 自定义牌堆不把旧 `MakecardConfig` 原样搬入 Battle。GameLogic 公开 `ProvideCustomDeck`，外部调用者先将文件、Redis 或其他格式转换成 canonical catalog，再以 BattleID 和小局身份推送；生产 Redis bridge 兼容旧 `game:makecard:<ProductID>` 优先、空值回退 `game:makecard:<GameID>` 的键和选择顺序。Legacy bridge 可由有界 `CustomDeckRunner` 执行读取、白名单和解析，但 Battle 不主动提交读取请求、不读取数据源配置。解析 grammar 保留参考行为：牌值 token 只需按十六进制解析为 `uint8`，允许重复和非标准编码；每块不足 104 项忽略，超过 104 项取前 104 项；庄家编号超出 `0..3` 视为未指定，token 或庄家文本无法解析则整次装载失败。Battle 只接受仍匹配当前小局的不可变 `CustomDeckCatalog` 并执行庄家/发牌规则；没有提供或 bridge 失败时回退普通洗牌，只记录脱敏日志和指标，不隔离牌局；迟到结果拒绝，运行中的小局不直接访问 Redis/Nacos/文件，也不观察外部目录的原地突变。
...
- Battle 异常必须按原因分类。普通 Handler 或 Timer Handler 的 panic、状态机不变量破坏和 Stop 超时属于程序缺陷，不自动重建、不回收 `BattleID`。GM 连接断开可以使尚未隔离的 Battle 进入 Stop 并在真实返回后回收，但不能覆盖已经进入 `Quarantined` 的条目。隔离条目收到 `DEL_GAME` 时只幂等记录外部已结束事实，不 Stop、不释放容量：首次观察保存由注入 Clock 产生且之后不可变的时间，重复消息只更新最近观察时间、观察次数和本次 ConnectionGeneration；同号 `NewGame` 按上一条返回 `Res=0`。这是相对参考实现的有意偏差：后续正常协议消息不能销毁缺陷证据。参考实现的 `OnTimer` 会 recover、记录日志并返回 `false`；新实现不复制该行为，因为 panic 发生时业务状态可能已部分更新。Go Runtime 无法安全抢占仍未返回的任意 Handler，Core 因此只通过慢 Command 指标和 `Runtime.Inspect().Tasks` 暴露它，不在 Handler 外并发修改 Service 或启动业务 watchdog；实际 panic、返回后的不变量失败及 Stop timeout 才进入可证明的隔离转换。
...
- 首版诊断导出使用组合根持有的本地文件 exporter，Core Runtime、Host 和 Battle Handler 不直接执行文件 I/O。exporter 把每份材料写入配置根目录下的独立临时目录，包含 `manifest.json`、`snapshot.json`、`commands.jsonl`、`panic.txt`、`runtime-inspection.json` 和最终 `receipt.json`；最后稳定 Snapshot、输入 Record、随机种子、Clock、失败 Command、Command Record Sequence、TurnRevision、stack、连接代际和 Runtime Inspection 等材料完整写出后，依次同步所有文件与临时目录、原子改名为 receipt 目录并同步父目录。只有这些步骤全部成功才生成不透明诊断 receipt。receipt 必须绑定 `BattleID`、完整 `BattleRef` 和导出材料 SHA-256 摘要；它只证明该实例的现场已经可靠保存，不自动改变 Host 状态或触发 Stop。运维人员拿到诊断材料后，显式提交完整 receipt；Host 精确校验成功后才通过生命周期 runner 停止仍存在的实例并释放条目。写入、同步或改名失败不得生成 receipt，旧实例 receipt 也不能用于释放复用相同编号的新实例。首版不依赖 MySQL、Redis 或对象存储；后续可替换 exporter，但不能改变 Host 与释放契约。
...
- 首版诊断运维面只提供权限为 `0600` 的节点本地 Unix socket 与 `cmd/nhskdiag`：列出隔离项、发起或重试导出、携 receipt 释放以及独立清理诊断目录。它不复用 Legacy GM TCP、不增加旧 MessageID，私有管理 Command 不进入 Cluster codec。远程运维必须等完整身份认证与授权契约明确后另行设计。
- 隔离前后必须保存诊断事实：`BattleID`、完整 `BattleRef`、连接代际、最后成功 Command、导致失败的 Command 元数据、Command Record Sequence、Subgame identity、窄业务 Revision、TurnRevision、最后稳定 Snapshot、固定随机种子、Clock、输入 Record、panic stack/稳定错误、Runtime Inspection 和相关日志。业务 Handler 不执行文件 I/O；组合根的有界诊断 runner 使用现有 Record/Snapshot seam 导出本地诊断包。诊断材料不得包含 token、secret、proof 或完整身份凭据。
